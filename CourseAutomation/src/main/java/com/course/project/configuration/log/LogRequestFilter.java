...
public class LogRequestFilter extends OncePerRequestFilter implements Ordered {
	...
	private void logTrace(Map<String, Object> trace) {
		String method = trace.get("method").toString();
		String path = trace.get("path").toString();
		String statusCode = trace.get("statusCode").toString();
		logger.info(String.format("%s %s '%s'. Trace: '%s'", method, path, statusCode, trace));
		customLogger.info(method, path, statusCode, trace.toString());
	}
